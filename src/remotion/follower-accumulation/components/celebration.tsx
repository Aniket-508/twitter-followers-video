...
export const Celebration: React.FC<CelebrationProps> = ({ theme, milestones }) => {
  ...
  const height = frame < celebrationStart ? 0 : interpolate(slideProgress, [0, 1], [0, LAYOUT.CELEBRATION_HEIGHT]);

  // Slide up from 30px below
  const translateY = frame < celebrationStart ? 30 : interpolate(slideProgress, [0, 1], [30, 0]);

  // Fade in
  const opacity = interpolate(frame, [celebrationStart, celebrationStart + fadeDuration], [0, 1], {
